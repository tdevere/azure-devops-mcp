...
export class BuildFailureReportGenerator {
  ...
  async generateReport(
    ...
    const [
      ...
      defaultOptions.includeEnvironmentDetails ? this.generateEnvironmentContext(build, defaultOptions) : Promise.resolve({} as EnvironmentContext),
  ...
  private async generateDiffAnalysis(build: Build, options: FailureReportOptions): Promise<BuildDiffAnalysis | undefined> {
    try {
      ...
      const successfulBuilds = await buildApi.getBuilds(
        this.project,
        [build.definition?.id!],
        undefined, undefined, undefined, build.queueTime,
  ...
  private async generateEnvironmentContext(build: Build, options: FailureReportOptions): Promise<EnvironmentContext> {
  ...
  private async generateRelatedIssues(build: Build, options: FailureReportOptions): Promise<RelatedIssue[]> {
    ...
    const recentFailures = await buildApi.getBuilds(
      ...
      BuildResult.Failed | BuildResult.PartiallySucceeded,
      undefined, undefined,
      options.maxRelatedIssues * 2 // Get more to filter
    );
  ...
  private async getTimelineRecords(build: Build): Promise<TimelineRecord[]> {
    try {
      const buildApi = await this.connection.getBuildApi();
  ...
  private analyzeFailureCategory(build: Build, timeline: TimelineRecord[]): FailureCategory {
  ...
  private analyzeFailureFrequency(recentBuilds: Build[]): FailureFrequency {
    const now = new Date();
  ...
  private calculateConfidenceScore(timeline: TimelineRecord[], failureType: FailureType): number {
  ...
  private async generateImpactAssessment(build: Build): Promise<ImpactAssessment> {
    return {
  ...
  private identifyPrimaryCause(failedTasks: TimelineRecord[]): string {
    if (failedTasks.length === 0) return "Unknown failure cause";
  ...
  private buildFailureStack(failedTasks: TimelineRecord[]) {
  ...
  private classifyError(failedTasks: TimelineRecord[]): ErrorClassification {
    return {
  ...
  private async getTaskLogs(build: Build, task: TimelineRecord): Promise<TaskLogEntry[]> {
  ...
  private extractEnvironmentVariables(task: TimelineRecord): Record<string, string> {
    // Extract environment variables from task if available
    return {};
  }

  private extractTaskInputs(task: TimelineRecord): Record<string, unknown> {
    // Extract task inputs if available
    return {};
  }

  private extractTaskOutputs(task: TimelineRecord): Record<string, unknown> {
    // Extract task outputs if available
    return {};
  }

  private async compareBuilds(currentBuild: Build, lastSuccessfulBuild: Build): Promise<BuildComparison> {
  ...
  private assessRisk(comparison: BuildComparison): RiskAssessment {
    return {
      overallRisk: "medium",
  ...
  private getTimelineStatus(record: TimelineRecord): "started" | "completed" | "failed" | "warning" {
    if (record.result === TaskResult.Failed) return "failed";
    if (record.result === TaskResult.Succeeded) return "completed";
    if (record.warningCount && record.warningCount > 0) return "warning";
    return "started";
  }
  ...
  private generateGeneralRecommendations(build: Build, timeline: TimelineRecord[]): IntelligentRecommendation[] {
    return [
      {
  ...
  private async calculateSimilarity(build1: Build, build2: Build): Promise<number> {
  ...
  private getSimilarityReason(similarity: number): string {
    if (similarity > 0.9) return "Nearly identical failure pattern";
    if (similarity > 0.7) return "Similar failure characteristics";
    if (similarity > 0.5) return "Some common failure elements";
  ...
  private getBuildReasonString(reason: any): string {
    // Convert build reason enum to string
    return reason?.toString() || "Unknown";
  }

  private getTaskStateString(state: any): string {
    return state?.toString() || "Unknown";
  }

  private getTaskResultString(result: any): string {
